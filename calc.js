$(document).ready(function() {
    ...
    var input = "";
    var equation = []; // Culmulative equation in array to hold user's entry.
    var MAX_INPUT = 8; // Max display amount.
    var MAX_EQUATION = 25; // Max culmulative display of entries.
    ...
    $(".equal").click(function() {
        ...
        if(input) {
            equation.push(input);
            $(".user-entry").html(parseFloat(eval(equation.join('')).toFixed(4)));
            $('.review').html(equation.join('') + "=" + parseFloat(eval(equation.join('')).toFixed(4)));
            input = '';
